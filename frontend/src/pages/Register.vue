...
async function handleRegister() {
  ...
  error.value = ''
  try {
    await auth.register(name.value, email.value, password.value, phone.value)
    router.push('/')
  } catch (err) {
...
<template>
  <div class="min-h-[60vh] flex items-center justify-center px-4 py-12">
    <div class="w-full max-w-md">
      <h1 class="text-2xl font-heading text-center mb-8">{{ t.auth.register }}</h1>
      ...
      <form @submit.prevent="handleRegister" class="space-y-4">
        <div>
          <label class="text-xs text-gray-500 mb-1 block">{{ t.auth.name }}</label>
        ...
        <div>
          <label class="text-xs text-gray-500 mb-1 block">{{ t.auth.confirmPassword }}</label>
      ...
      <p class="text-center text-sm text-gray-500 mt-6">
        {{ t.auth.hasAccount }}
        <router-link to="/login" class="text-accent underline">{{ t.auth.login }}</router-link>
      </p>
    </div>
  </div>
</template>
